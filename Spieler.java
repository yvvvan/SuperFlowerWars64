...
// --request -> confirm -> update ----------------------------------------------
    ...
    public Move request() throws Exception, RemoteException{
      if(check==1){
        ...
        if (possiblemoves.contains(myMove)){
          board.make(myMove);
          check=2;
          return myMove;
        }else{
          throw new Exception("Ihre Move ist nicht gültig!");
        }
      }else{
...
// --getter & setter------------------------------------------------------------
	...
	* @return board
	*/
	public SRWBoard getBoard() {
		return board;
	}

	/**
	* Sets new value of board
	* @param SRWBoard [eines neues Board
	*/
	public void setBoard(SRWBoard board) {
		this.board = board;
	}
